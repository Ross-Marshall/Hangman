...
def close_image():
    cmd = "`ps -ef  | grep \'display \' | grep -v grep | grep megazor | awk \'{print \"kill -9 \" $2}\'`"
    os.system( cmd )
...
cmd_template="nohup eog /me/code/python/hangman/img<<INDEX>>.png &"
cmd=cmd_template.replace("<<INDEX>>",str(1))
os.system(cmd)
...
while body_index < len( hangman ):
    ...
    if a_letter not in hangman_word:
        ...
        #if body_index > 0:
        #    close_image()
        cmd=cmd_template.replace("<<INDEX>>",str( body_index + 1 ))
        os.system(cmd)
